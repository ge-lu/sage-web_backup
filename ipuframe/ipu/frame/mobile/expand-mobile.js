 ...
 */
// define(["require", "jcl"], function (require, jcl) {
import jcl from '../base/jcl';

const ExpandMobile = function (execute, storageCallback) {
  return {
    ...
    getContactsView: function (callback, data, setting, err) {
      if (data == undefined) {
        data = new jcl.DataMap();
      }
      if (setting == undefined) {
        setting = new jcl.DataMap();
      }
    ...
    sms: function (sn, msg, autoSms, err, callback) {
      if (autoSms == undefined) {
        autoSms = false;// false-跳转至短信界面,true-直接短信
      }
      storageCallback('sms', callback);
    ...
    markMap: function (callback, markParam, isSelect, isJump, err) {
      if (typeof (markParam) === 'object' && (markParam instanceof jcl.DataMap)) {
        const markParams = new jcl.DatasetList();
    ...
    execSQL: function (dbName, sql, bindArgs, limit, offset, callback, err) {
      ...
      execute('execSQL', [
        dbName, sql, bindArgs.toString(), limit, offset
      ], err);
    },
    insert: function (dbName, table, datas, callback, err) {
      if (datas == undefined) {
        datas = new jcl.DataMap();
      }
    ...
    update: function (dbName, table, datas, condSQL, conds, callback, err) {
      ...
      execute('update', [
        dbName, table, datas.toString(), condSQL, conds.toString()
      ], err);
    },
    select: function (dbName, table, columns, condSQL, conds, limit, offset, callback, err) {
      ...
      execute('select', [
        dbName, table, columns, condSQL, conds.toString(), limit, offset
      ], err);
    },
    ...
    downloadFile: function (servletUrl, savePath, callback, err) {
      storageCallback('downloadFile', callback);
      execute('downloadFile', [servletUrl, savePath], err);
    },
    ...
    lbsNearbySearch: function (ak, geoTableId, q, loc, radius, err) {
      execute('lbsNearbySearch', [
        ak, geoTableId, q, loc, radius
      ], err);
    },
    ...
    setImageWithURLs: function (urls, callback, forceDownload) {
      storageCallback('setImageWithURLs', callback);
      execute('setImageWithURLs', [urls, forceDownload]);
    },
    ...
    openOuterApp: function (androidPkg, clsName, iosUri, androidUrl, iosUrl, params, callback) {
      ...
      execute('openOuterApp', [
        androidPkg, clsName, iosUri, androidUrl, iosUrl, params
      ]);
    },
...
//   if (!IpuMobile) {
//     // webpack打包写法
//     IpuMobile = require("./ipu-mobile").default;
//
//     // vue3 vite打包写法
//     // IpuMobile = import.meta.globEager('./ipu-mobile.js')['./ipu-mobile.js'].default;
//   }
//   return IpuMobile;
// }
//
...
//   getIpuMobile().callback.storageCallback(action, callback);
// }
