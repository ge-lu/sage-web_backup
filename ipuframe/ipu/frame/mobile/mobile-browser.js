 ...
 */
// define(["browserTool", "jcl"], function (browserTool, jcl) {
...
const Mobile = new function () {
  ...
  /** ****************内存缓存**********************/
  this.setMemoryCache = function (key, value, sync) {
    executeSync(function () {
      browserTool.browser.setMemoryCache(key, value);
    }, sync);
  };
  // 编码参参数isEncrypt未使用，sync在些情况下需要使用同步调用，默认为异常调用
  this.getMemoryCache = function (callback, key, value, isEncrypt, sync) {
    executeSync(function () {
      browserTool.browser.getMemoryCache(callback, key, value);
    }, sync);
  };
  this.removeMemoryCache = function (key, sync) {
    executeSync(function () {
      browserTool.browser.removeMemoryCache(key);
    }, sync);
  };
  this.clearMemoryCache = function (sync) {
    executeSync(function () {
      browserTool.browser.clearMemoryCache();
    }, sync);
  };
  /** ****************离线缓存**********************/
  this.setOfflineCache = function (key, value, isEncrypt, sync) {
    executeSync(function () {
      browserTool.browser.setOfflineCache(key, value, isEncrypt);
    }, sync);
  };
  this.getOfflineCache = function (callback, key, value, isEncrypt, sync) {
    executeSync(function () {
      browserTool.browser.getOfflineCache(callback, key, value, isEncrypt);
    }, sync);
  };
  this.removeOfflineCache = function (key, sync) {
    executeSync(function () {
      browserTool.browser.removeOfflineCache(key);
    }, sync);
  };
  this.clearOfflineCache = function (sync) {
    executeSync(function () {
      browserTool.browser.clearOfflineCache();
    }, sync);
  };
  /** ****************扩展UI**********************/
  let windowCallback;// 关闭窗口时的回调函数
  let windowFlag = false;// 关闭窗口的标识
  /* 打开窗口 */
  this.openWindow = function (pageAction, param, callback) {
    if (param) {
      param = { data: param }; // 转换json格式
    }
    const url = browserTool.redirect.buildUrl(pageAction, null, true);
  ...
  let dialogCallback;// 关闭对话框时的回调函数
