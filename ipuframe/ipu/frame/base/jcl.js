...
import $ from '../../lib/zepto/zepto.min-1.1.6';
...
$.Collection.prototype = {
  ...
  each: function (fn, scope) {
    const items = [].concat(this.items); // each safe for removal
  ...
  item: function (key) {
    ...
    const item = mk !== undefined ? mk : (typeof key === 'number') ? this.items[key] : undefined;
  ...
  keySort: function (dir, fn) {
    this._sort('key', dir, fn || function (a, b) {
      const v1 = String(a).toUpperCase(); const v2 = String(b).toUpperCase();
  ...
  filterBy: function (fn, scope) {
    const r = new $.Collection();
    r.getKey = this.getKey;
    const k = this.keys; const it = this.items;
...
$.extend($, {
  ...
  each: function (object, callback, args) {
    ...
    const isObj = length === undefined || $.isFunction(object);
...
$.extend($.DataMap.prototype, {
  ...
  parseString: function (str) {
    ...
    return window.JSON && window.JSON.parse
      ? this.parseObject(window.JSON.parse(str))
...
$.DatasetList = function (o) {
  ...
  if (typeof (o) === 'object' && (o instanceof Array) && o.length) {
    this.parseArray(o);
  }
};
$.extend($.DatasetList.prototype, {
  ...
  each: function (fn, scope) {
    ...
    for (let i = 0, len = items.length; i < len; i = i + 1) {
      if (fn.call(scope || items[i], items[i], i, len) === false) {
        break;
      }
    }
  ...
  indexOf: function (o) {
    if (!this.items.indexOf) {
      for (let i = 0, len = this.items.length; i < len; i = i + 1) {
        if (this.items[i] == o) {
          return i;
        }
      }
  ...
  parseString: function (str) {
    ...
    return window.JSON && window.JSON.parse
      ? this.parseArray(window.JSON.parse(str))
...
$.extend($, {
  ...
  ajaxRequest: function (origSettings) {
    ...
    const s = $.extend(true, {}, $.ajaxSettings, origSettings);
    // 定义变量
    let jsonp; let status; let data;
    ...
    if (s.cache === false && type === 'GET') {
      const ts = now();
      // 尝试替换"_="，即原有的随机参数
      const ret = s.url.replace(rts, '$1_=' + ts + '$2');
      // 如果替换失败，则拼入随机参数
    ...
    const remote = parts && (parts[1] && parts[1] !== location.protocol || parts[2] !== location.host);
